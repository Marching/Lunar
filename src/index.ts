...
export class SolarTermCalculator {
  ...
  static calcTerm(year: number, termNO: number, timeOffset: number): Date {
    ...
    // Step 1
    t = 0;
    const lng0 = SolarTermCalculator.getLng(t, ACCURACY.LOW);

    // Step 2
    t = t + (solarLng - lng0) / SolarTermCalculator.getSolarLngAvgSpeed();
    const v1 = SolarTermCalculator.getSolarLngInsSpeed(t);

    // Step 3
    t = t + (solarLng - SolarTermCalculator.getLng(t, ACCURACY.MIDDLE)) / v1;

    // Step 4
    t = t + (solarLng - SolarTermCalculator.getLng(t, ACCURACY.HIGH)) / v1;
    ...
    let yearCd: number = 0;
    let monthCd: number = 0;
    let dayCd: number = 0;
    let hourCd: number = 0;
    let minuteCd: number = 0;
    let secondCd: number = 0;
