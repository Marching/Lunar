import { ISolarTerm, SOLAR_TERMS } from './terms';
 ...
 *
 */
export class ISolarTermCalculator {
  static J2000 = 2451545;

  static getTerms(year: number): ISolarTerm[] {
    const timeOffset = new Date().getTimezoneOffset() / 60;
    return SOLAR_TERMS.map((item, index) => {
      return {
        date: ISolarTermCalculator.calcTerm(year, index + 1, timeOffset),
        lunar: '',
  ...
  static calcTerm(year: number, termNO: number, timeOffset: number): Date {
    ...
    // Step 1
    t = 0;
    const lng0 = ISolarTermCalculator.getLng(t, ACCURACY.LOW);

    // Step 2
    t = t + (solarLng - lng0) / ISolarTermCalculator.getSolarLngAvgSpeed();
    const v1 = ISolarTermCalculator.getSolarLngInsSpeed(t);

    // Step 3
    t = t + (solarLng - ISolarTermCalculator.getLng(t, ACCURACY.MIDDLE)) / v1;

    // Step 4
    t = t + (solarLng - ISolarTermCalculator.getLng(t, ACCURACY.HIGH)) / v1;
    const julianDay = ISolarTermCalculator.J2000 + t * 36525 - ISolarTermCalculator.calcDTDeltat(year, t) / 86400 - timeOffset / 24;
  ...
  static calcDTDeltat(year: number, dynamicalTime: number): number {
    ...
    if (
      ...
      if (year >= 2005 && year < 2014) {
        ...
        value = ((year - 2114) * (ISolarTermCalculator.calcDTDeltat(2114, dynamicalTime) - ISolarTermCalculator.calcDTDeltat(2014, dynamicalTime))) / 100;
      } else {
  ...
  static getSolarLngInsSpeed(dynamicalTime: number): number {
    ...
    const solarLng = Math.round(this.getSolarLngAvgSpeed() * 1000) / 1000;
